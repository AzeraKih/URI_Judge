...
-- A saída deve estar agrupada por departamento e divisão. Dentro de cada divisão, a lista de empregados deve estar de forma decrescente por salário líquido.
...
-- v2
WITH s AS (
select matr m, sum(v) v, sum(d) d from (
select e.matr, coalesce(valor, 0) v, 0 d from emp_venc ev
	join vencimento v
    on v.cod_venc = ev.cod_venc
    right join empregado e
	on e.matr = ev.matr
union all
select matr m, 0 v, valor d from emp_desc ed
	join desconto d on d.cod_desc = ed.cod_desc
) s group by matr),
e as (
    select e.matr m, e.nome n, d.nome d from
        empregado e
    join divisao dv
        on dv.cod_divisao=e.lotacao_div
    join departamento d
        on d.cod_dep=dv.cod_dep
)
select e.d Departamento,e.n Empregado, s.v "Salario Bruto", s.d "Total Desconto", s.v - s.d "Salario Liquidoaws"
    from s
  join e
	on e.m = s.m
  order by 5 desc;
...
--v1
WITH sa AS (
select matr, sum(v) v, sum(dc) dc from (
select e.matr, coalesce(valor, 0) v, 0 dc from emp_venc ev
	right join vencimento v
    on v.cod_venc = ev.cod_venc
    right join empregado e
	on e.matr = ev.matr
union all
select matr, 0 venc, valor as dc from emp_desc
	join desconto d on d.cod_desc = emp_desc.cod_desc
) s group by matr
)
select de.nome Departamento, emp.nome Empregado, sa.v "Salario Bruto", sa.dc "Total Desconto", sa.v - sa.dc "Salario Liquidoaws"
    from sa
  join empregado emp
	on sa.matr = emp.matr
  join divisao div
  ...
  join departamento de
	on de.cod_dep = div.cod_dep
  order by 5 desc;




WITH s AS (select matr m, sum(v) v, sum(d) d from (select e.matr, coalesce(valor, 0) v, 0 d from emp_venc ev join vencimento v on v.cod_venc = ev.cod_venc right join empregado e on e.matr = ev.matr union all select matr m, 0 v, valor d from emp_desc ed join desconto d on d.cod_desc = ed.cod_desc) s group by matr), e as (select e.matr m, e.nome n, d.nome d from empregado e join divisao dv on dv.cod_divisao=e.lotacao_div join departamento d on d.cod_dep=dv.cod_dep) select e.d Departamento,e.n Empregado, s.v "Salario Bruto", s.d "Total Desconto", s.v - s.d "Salario Liquidoaws" from s join e on e.m = s.m order by 5 desc;
