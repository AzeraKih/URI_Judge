...
--departamento   |           divisao            |  media  |  round   
-----------------+------------------------------+---------+----------
-- TI            | Analise de Sistemas          | 7733.33 | 10350.00
-- TI            | Programacao                  | 6900.00 |  7200.00
-- Engenharia    | Concreto                     | 6316.67 | 10650.00
-- TI            | Desenvoilvimento de Projetos | 6266.67 |  7700.00 ##
-- Contabilidade | Passivo                      | 5900.00 |  8700.00
-- Engenharia    | Calculo Estrutural           | 5600.00 |  6700.00
-- Contabilidade | Ativo                        | 4366.67 |  6000.00
...
select departamento.nome as departamento, divisao.nome as divisao, round(sum(valor)/count(1),2) as media, round(max(valor),2) as maior from (
	select matr, sum(valor) as valor from (
	select empregado.matr, sum(coalesce(valor, 0)) as valor from emp_venc
	...
	union all
	select matr, sum(valor) * -1 as valor from emp_desc
		join desconto on desconto.cod_desc = emp_desc.cod_desc
		group by matr) as venc_desc
	group by matr
) as salario join empregado 
	on salario.matr = empregado.matr
  join divisao 
...
select departamento.nome as departamento, divisao.nome as divisao, round(sum(valor)/count(1),2) as media, round(max(valor),2) as maior from salaries join empregado 
