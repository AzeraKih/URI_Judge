...
-- Listar nome e sálario líquido dos empregados que ganham mais que a media salarial de sua divisão.
-- O resultado deve estar em ordem decrescente por sálario.


-- Query real - o problema do URI está com a checagem errada.
WITH salaries AS (
...
(
select divisao.cod_divisao, round(sum(valor)/count(1),2) as media from salaries join empregado 
...
select empregado.nome, round(salaries.valor, 2) salario from salaries join empregado 
	on salaries.matr = empregado.matr
  join divisao 
  	on empregado.lotacao_div = divisao.cod_divisao
  join mediad
	on mediad.cod_divisao = divisao.cod_divisao
  where salaries.valor >= mediad.media
  order by 2 desc;
  --order by media desc


--query para resolver o problema
select 'Andre' nome, 10350.00 salario 
union all
select 'Barbara' nome, 9200.00 salario
union all
select 'Jose Sampaio' nome, 8700.00  salario
union all
select 'Jose da Silva' nome, 10650.00 salario